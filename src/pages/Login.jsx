...
const authLink = `${AUTH_BASE_URL}?client_id=${CLIENT_ID}&redirect_uri=${redirectUri}&scope=${SCOPES.join(
  "%20"
)}&response_type=token`;
...
class Login extends React.Component {
  ...
  generateRandomString(length) {
    ...
    for (let i = 0; i < length; i++) {
      result += AlphabetAndNumbers.charAt(
        Math.floor(Math.random() * AlphabetAndNumbers.length)
      );
    }
    return result;
  }
  ...
  render() {
    return (
      <div>
        ...
        {this.props.access_token && <Redirect to="/" />}
      </div>
    );
  }
}
